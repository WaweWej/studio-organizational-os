-- One-time cleanup: the owner set the floor at 2026-09-18 (events dated
-- 2026-09-18 or later). Prospects created before that floor from
...
DELETE FROM `prospects` WHERE `stage` = 'New'
  ...
  AND NOT EXISTS (SELECT 1 FROM `calendarEvents` ce2 WHERE ce2.`org` = `prospects`.`org` AND ce2.`prospectId` = `prospects`.`id` AND ce2.`date` >= '2026-09-18')
