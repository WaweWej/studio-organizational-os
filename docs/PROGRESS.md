...
Prepared the existing Sites registration for owner-only production with persistent DB/ASSETS bindings. Local preview is clearly labeled, separate from hosted records. Added a bounded, authenticated, secret-gated one-time importer, immutable transfer receipt migration 0015, consistent local SQLite backup helper, and export helper. The importer preserves canonical IDs and history while remapping the authenticated organization; it rejects nonempty targets and files/vault records, rolls back atomically and deduplicates identical retries. Data and temporary credentials never enter source or deployment archives. Deployment and transfer completion are recorded below once verified.
