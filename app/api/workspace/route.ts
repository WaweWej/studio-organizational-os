...
export async function POST(request:Request){try{sameOrigin(request);if(!request.headers.get('content-type')?.startsWith('application/json'))throw new AppError('JSON is required.',415);if(Number(request.headers.get('content-length')||0)>100000)throw new AppError('Request too large.',413);const raw=await request.text();if(raw.length>100000)throw new AppError('Request too large.',413);let input;try{input=JSON.parse(raw)}catch{throw new AppError('Invalid JSON.')}if(!input||typeof input!=='object'||Array.isArray(input))throw new AppError('An action object is required.');const c=await context();await mutate(c,input);return response(await readWorkspace(c))}catch(e){return failure(e)}}
